...
int main() {
	...
	//printPath(S);
	int tableau[4] = { 0 };
	readFile("../algo_labyrinthe/CaCLC.csv", tableau);
	int SupppppppperSize, SupppppppperTeleporteur, SupppppppperGarde;
	int SuppppppppperTheme;
	/*for (int i = 0; i < 4; i++) {
		printf("\nValeur : %d\n", tableau[i]);
	}*/
	SupppppppperSize = tableau[0];
	SupppppppperTeleporteur = tableau[1];
	SupppppppperGarde = tableau[2];
	SuppppppppperTheme = tableau[3];
	printf("\nsize : %d", SupppppppperSize);
	printf("\nTeleporteur : %d", SupppppppperTeleporteur);
	printf("\nGrade : %d \nTheme : %d", SupppppppperGarde, SuppppppppperTheme);
	printf("\n");
